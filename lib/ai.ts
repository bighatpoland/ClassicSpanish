...
export async function generateSpeakingPrompt(payload: AiPromptRequest): Promise<{ enabled: boolean; text: string }> {
  ...
  const text = await requestOpenAi(
    "You generate one short beginner-friendly Spanish speaking prompt for a solo learner. Return in Polish with one Spanish prompt and three compact support bullets.",
    `Temat: ${payload.topic ?? "codziennosc"}. Tryb: ${payload.mode ?? "voice"}. Frazy: ${(payload.recentPhrases ?? []).join("; ")}`
  );
...
export async function generateTextFeedback(payload: AiFeedbackRequest): Promise<{ enabled: boolean; text: string }> {
  ...
  const text = await requestOpenAi(
    "You provide concise supportive feedback for a Spanish learner. Return in Polish with one encouragement sentence, three practical corrections, and one tiny next step.",
